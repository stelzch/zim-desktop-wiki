...
class SourceViewBuffer(GtkSource.Buffer):
        ...
        def __init__(self, attrib, text):
                ...
                if attrib['scheme']:
                        self._set_style_scheme(attrib['scheme'])
        ...
        def set_style_scheme(self, scheme):
                self.object_attrib['scheme'] = scheme
                self._set_style_scheme(scheme)
                self.emit('changed')

        def _set_style_scheme(self, scheme):
                try:
                        GtkSource.Buffer.set_style_scheme(self, sm.get_scheme(scheme))
                except:
                        logger.exception('Could not set style scheme for sourceview: %s', scheme)
...
class SourceViewWidget(TextViewWidget):
        ...
        def on_populate_popup(self, view, menu):
                ...
                def activate_scheme(item):
                        self.buffer.set_style_scheme(item.zim_sourceview_scheme)

                item = Gtk.MenuItem.new_with_mnemonic(_('Scheme'))
                item.set_sensitive(self.view.get_editable())
                submenu = Gtk.Menu()
                for scheme in sorted(SCHEMES, key=lambda k: k.lower()):
                        schemeitem = Gtk.MenuItem.new_with_mnemonic(scheme)
                        schemeitem.connect('activate', activate_scheme)
                        schemeitem.zim_sourceview_scheme = scheme
                        submenu.append(schemeitem)
                item.set_submenu(submenu)
                menu.prepend(item)
